...
- **Description** : Identité des élus (nom, prénom, date de naissance, photo, slug permalink) et historique de leurs mandats (législature, circonscription, dates de début/fin). Couvre les députés, sénateurs, maires, conseillers départementaux, conseillers régionaux, conseillers d'arrondissement et membres des assemblées territoriales à statut particulier. Le slug (ex. `manuel-bompard`) est généré automatiquement à l'insertion et sert de permalink pour les URLs (`/elus/{slug}`). Les homonymes sont suffixés (`-1`, `-2`).
- **Types de mandats** : `depute`, `senateur`, `maire`, `eurodepute`, `conseiller_departemental`, `conseiller_regional`, `conseiller_arrondissement`, `membre_assemblee_statut_particulier`
...
- **Source membres des assemblées des collectivités à statut particulier** :
  - RNE (data.gouv.fr) — fichier CSV dédié (~557 entrées, 10 collectivités : Corse, Guyane, Martinique, Métropole de Lyon, Nouvelle-Calédonie, Polynésie française, Saint-Barthélemy, Saint-Martin, Saint-Pierre-et-Miquelon, Wallis-et-Futuna), séparateur `;`, UTF-8, CRLF, mise à jour trimestrielle
  - URL : `https://www.data.gouv.fr/api/1/datasets/r/a595be27-cfab-4810-b9d4-22e193bffe35`
  - Colonnes (19, la plus riche des sources RNE locales) : Code/Libellé région, Code/Libellé département, Code/Libellé collectivité à statut particulier, Code/Libellé section (toujours vide dans les données actuelles), Code/Libellé circonscription métropolitaine, Nom, Prénom, Code sexe, Date naissance, Code CSP, Libellé CSP, Date début mandat, Libellé de la fonction (vide, ou « Président »/« Vice-président »/« Membre de l'organe exécutif » — non persisté), Date début fonction
  - 9 des 10 collectivités (toutes sauf la Métropole de Lyon) forment une assemblée unique sans subdivision : région/département/circonscription sont vides, seuls le code/libellé de la collectivité sont renseignés. Toutes sont déjà reconnues comme "département" ailleurs sur le site via `DEPARTMENT_CODES` (Guyane, Martinique, Polynésie française, Nouvelle-Calédonie, Wallis-et-Futuna, Saint-Barthélemy, Saint-Martin) sauf la Corse (scindée en 2A/2B dans `DEPARTMENT_CODES`, mais regroupée en une seule collectivité "Corse" dans cette source). Pour ces 9, `mandates.department` = `mandates.district` = le nom de la collectivité.
  - La Métropole de Lyon fait exception : elle a de vraies circonscriptions internes (ex. `69001` "Lones Et Coteaux"), un vrai département de rattachement (`Rhône`) et une région (`Auvergne-Rhône-Alpes`) — `mandates.district` = nom de la circonscription, `mandates.department` = `Rhône`. À ne pas confondre avec les conseillers régionaux élus sur la section départementale "Métropole de Lyon" (scindée du Rhône côté conseil régional) : ce sont deux assemblées distinctes qui partagent le même nom de territoire.
  - Certaines personnes apparaissent sur plusieurs lignes (changement de fonction, ex. élu·e puis devenu·e président·e de l'assemblée) — sans conséquence : la fonction n'étant pas persistée, l'upsert converge simplement vers la dernière ligne rencontrée pour ce mandat.
  - Client : `sources/rne-membres-assemblee.ts` → `upsert/membres-assemblee.ts`. Même adaptation que les conseillers régionaux/arrondissement (matching par paire (territoire, official), N titulaires par territoire).
